...
resource "aws_cloudwatch_log_group" "ecs_cluster" {
  name = "ecs-cluster-log-${var.account_id}-${var.env}"
}

resource "aws_ecs_cluster" "workflow_cluster" {
  name = "workflow-cluster-${var.account_id}-${var.env}"
...
resource "aws_ecs_task_definition" "workflow-manager" {
  family                = "wm-${var.account_id}-${var.env}"
  ...
  container_definitions = jsonencode([
    {
      name      = "wm-${var.account_id}-${var.env}"
      ...
      mountPoints = [
        {
          sourceVolume = "wm-storage-${var.account_id}-${var.env}"
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          awslogs-group = "/ecs/wm/${var.account_id}-${var.env}"
  ...
  volume {
    name = "wm-storage-${var.account_id}-${var.env}"
